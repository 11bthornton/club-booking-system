...
export function ClubModal({
    ...
    const filteredClubs = Object.values(availableClubs).filter((club) =>
        Object.values(club.club_instances).some(
            (instance) =>
                instance.half_term === term && instance.day_of_week === day,
        ),
    );

    const alreadySelectedAsIds = clubSelections.flatMap((obj) =>
        Object.values(obj)
            .map((day) => day && day.id)
            .filter(Boolean),
    );
        ...
        >
            ...
            >
                <div className="flex justify-center items-baseline">
                    <h2
                        id="club-modal-title"
                        className="justify-center font-bold text-4xl"
                    >
                ...
                <Box
                    style={{ marginBottom: "20px", padding: "0px 20px" }}
                ></Box>
                ...
                >
                    ...
                    >
                        {filteredClubs.map((club) => {
                            ...
                            const isDisabled = alreadySelectedAsIds.some(
                                (id) => instance.id === id,
                            );
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="flex justify-between items-baseline">
                                        <div>
                                            {alreadySelectedAsIds.some(
                                                (id) => instance.id == id,
                                            ) ? (
                                                <Alert severity="success">
                                                    <p className="font-bold">
                                                        You already have a place
                                                        on this club for this
                                                        day!
                                                    </p>
                                                </Alert>
                                            ) : (
                                            ...
                                            {alreadySelectedAsIds.some((id) =>
                                                instance.incompatible_club_ids.includes(
                                                    id,
                                                ),
                                            ) ? (
                                                <Alert
                                        ...
                                        <Button
                                            ...
                                            onClick={async () => {
                                                try {
                                                    const changeInfo =
                                                        await findClubChanges(
                                                            instance.id,
                                                        );
                                                    setSelectedChangeInfo(
                                                        changeInfo.data,
                                                    );
                                                    setSelectedInstanceToBook(
                                                        instance.id,
                                                    );
                                                    setChangeInfoModalOpen(
                                                        true,
                                                    );
                                                } catch (error) {
                                                    console.error(
                                                        "Error fetching change info:",
                                                        error,
                                                    );
                                                }
                                            }}
